...
(function (win, doc) {
  ...
  $wt.piwik = {
    ...
    run: function (j) {
      var cook = false,
        ref = location.href;
      (isFound = false), (canBeProcess = false);
      ...
      if (!j["siteID"]) {
        ...
        if (j["sitePath"]) {
          ...
          for (var i = 0, l = cook.length; i < l; i++) {
            ...
            // remove the ending "/"
            if (
              cookCheck.slice(cookCheck.length - 1, cookCheck.length) === "/"
            ) {
            ...
            isFound =
              ref.indexOf(cookCheck) !== -1 && ref.indexOf(cookCheck) < 9;
        ...
        } else if ($wt.getDocLang(true) !== false) {
          // UEC
        ...
        if (j["search"]) {
          ...
          var category = j["search"]["category"]
            ? j["search"]["category"]
            : false;
        ...
        if (canBeProcess) {
          ...
          if (instance_status) {
            ...
            console.log(
              "WTERROR: [PIWIK] The Piwik instance, [" +
                instance +
